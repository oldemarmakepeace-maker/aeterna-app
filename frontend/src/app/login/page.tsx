    ...
    <div className="min-h-dvh flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="font-sans text-xs text-text-muted uppercase tracking-widest">Life Assistant</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          >
            {loading ? <div className="w-5 h-5 border-2 border-surface-1 border-t-transparent rounded-full animate-spin" /> : "Войти"}
          </motion.button>
        </form>

        <p className="mt-8 text-center text-xs text-text-secondary font-sans border-t border-gunmetal/30 pt-6">
          Нет аккаунта?{" "}
          <Link href="/register" className="text-copper hover:text-copper-light transition-colors">
            Регистрация
          </Link>
        </p>
      </motion.div>
